...
请根据用户的最新反馈，做三件事：
1. 总结用户反馈的核心诉求（一句话，20字内）
2. 给出当前方案的改进方向建议（每条20字内）——必须是具体可落地的方向描述，不是提问句
3. 提供 2-3 个具体选项让用户选择——每个选项是具体的方向/功能/改进点，用名词短语而不是问句

重要：选项不要写成疑问句！写成："加入社交排行榜" "增加阅读时长统计" "改成多人在线PK" 这样的具体落地方向，方便用户直接点击选择。

输出格式为 JSON：
{
  "version_update": "v${version+1}.0 建议（30字以内，描述本轮迭代的核心改进方向）",
  "improvement_ideas": ["具体改进方向1（20字内，不要问句）", "改进方向2（20字内）"],
  "next_options": ["选项1：具体方向描述（15字内）", "选项2：具体方向描述", "选项3：具体方向描述"],
...
只输出 JSON，不要其他内容。`;
  ...
  return {
    ...
    improvement_ideas: ["加入社交传播机制", "增强互动趣味性"],
    next_options: ["加入排行榜/成就系统", "增加社交分享/邀请", "优化核心交互流程"],
    new_direction_hint: ""
  };
}
